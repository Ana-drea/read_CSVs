...
def sum_csv(path):
    df_1 = pd.read_csv(path, encoding="utf-8", sep=';', header=1, usecols=[4, 14],
                       names=["Context Match", "100%"])
    df_2 = pd.read_csv(path, encoding="utf-8", sep=';', header=1, usecols=[9, 19],
                       names=["Repetitions", "95% - 99%"])
    df: DataFrame | Series = pd.concat([df_1, df_2], axis=1)
    # sum the 85% - 94% (Words) & 75% - 84% (Words) into 75%-95% Fuzzy
    df["75%-95% Fuzzy"] = pd.read_csv(path, encoding="utf-8", sep=';', header=1, usecols=[24, 29]).sum(axis=1)
